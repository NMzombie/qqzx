(function($) {
    ...
    $.fn.BKframe = function(options) {
        ...
        var defaults = {
            ...
            data: null, //数据
            mouIconOpen: "iconfont icon-xia", // 菜单
            mouIconClose: "iconfont icon-shang", // 菜单
            iconsort:'&#xe634;',//图文展示栏目
            iconsortg:'&#xe635;',//图标展示栏目
            menuopt: '', //栏目显示选择模式
        ...
        return this.each(function() {
            ...
            if(settings.bkposition == "left") {
                ...
                var titlename = '<div class="title-menu"><span class="'+settings.iconfont+' " id="menuclick" ischek="false">'+iconsort+'</span>' + settings.titlename + '</div>';
                ...
                if(mobile_flag) {
                    ...
                    footerform.css({top:(0)}).addClass("footer-top");
                ...
                $(settings.menuModule).addClass("leftModule").css({
                    width: settings.menuWidth,
                    height: ($(window).height() - ((settings.hrederheight) + (settings.footerheight)))+50,
                    float: "left",
                });
            ...
            function currentTime() {
                ...
                if (d.getHours() < 10) {
                    str += '<span class="h com-time ">0' + d.getHours() + '</span>' + '<b class="">：</b>';
                } else {
                    str += '<span class="h com-time ">' + d.getHours() + '</span>' + '<b class="">：</b>';
                }
                if (d.getMinutes() < 10) {
                    str += '<span class="m com-time ">0' + d.getMinutes() + '</span>' + '<b class="">：</b>';
                } else {
                    str += '<span class="m com-time ">' + d.getMinutes() + '</span>' + '<b class="">：</b>';
                }
                if (d.getSeconds() < 10) {
                    str += '<span class="s com-time ">0' + d.getSeconds() + '</span></div>';
                } else {
                ...
                clickBombbox:function(e){
                    ...
                    if(target.is(settings.prompt_btn)) {
                        if($(settings.prompt_btn).hasClass("promptclose")) {
                            ...
                            $(settings.prompt_btn).attr('class', 'dropdown '+settings.iconfont+'icon-jiantoushang').removeClass("promptclose");
                        } else {
                            ...
                            $(settings.prompt_btn).attr('class', 'dropdown '+settings.iconfont+'icon-jiantou').addClass("promptclose");
                        }
                    }
                    if(!target.is(settings.prompt_btn)) {
            ...
            function creatIframe(href, titleName,id, pageid) {
                ...
                if(id==2){
                    show_nav.append('<li class="home active"><span data-id="'+pageid+'" data-title="' + titleName + '" data-href="' + url + '">' + titleName + '</span><em class="close_icon"></em></li>');
                }else{
            ...
            $(document).on("click", "#tab-navigation li", function() {
                ...
                var menuSlideCountmap = {};
                if(menu !=null){
                    for(var i=0;i<menu.length;i++){
                    	if(menuSlideCountmap[menu[i].id]==undefined){
                    		menuSlideCountmap[menu[i].id] = 1;
                    	}

                    	 var value = menuSlideCountmap[menu[i].id];
                    	 if(value==1){
                    		 if($(this).find('span').attr('data-title') == menu[i].title){
                                 munlist.each(function(e){
                                     ...
                                     if(title==menu[i].title){
                                         munlist.eq(e).parent().parent().addClass("open").slideDown("fast").attr("ischek", 'true');
                                         munlist.eq(e).parent().parent().find('.submenu').slideDown("fast");
                                         munlist.eq(e).parent().parent().find("i").attr("ischek", 'true');
                                         munlist.eq(e).addClass("open").slideDown("fast").attr("ischek", 'true');
                                         ...
                                         munlist.eq(e).find('.submenu').slideUp("fase");
                                         munlist.eq(e).find("i").attr("ischek", 'false');
                                         munlist.eq(e).parent().parent().find("i").attr("ischek", 'false');
                                     }
                                 });
                                //已滑动
                                 menuSlideCountmap[menu[i].id] =  menuSlideCountmap[menu[i].id] + 1;
                                 menu[i].nate==2 ? menu[i].nate=2 : menu[i].nate=1;
                             }else{
                                 menu[i].nate=0;
                             }
