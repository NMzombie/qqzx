        ...
        success: (res) => {
            ...
            for (let i = 0; i < data.list.length; i++) {
                if (data.list[i].articleType === 0) {
                    if (dynamic === 0) {
                        firArr += `<div class="news-item-img"><a href="${baseUrl}/doc/wap/news-${data.list[i].id}.htm"><img src="${data.list[i].picUrls}" width="100%" height="100%" alt="${data.list[i].title}"></a></div><a href="${baseUrl}/doc/wap/news-${data.list[i].id}.htm"><div class="news-text-topic">${data.list[i].title}</div></a>`
